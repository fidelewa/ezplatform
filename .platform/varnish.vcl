...
sub ez_purge_acl {
    if (req.http.x-purge-token) {
        if (req.http.x-purge-token != req.http.x-backend-purge-token) {
...
// Sub-routine to get purge token.
sub ez_invalidate_token {
    ...
    if (req.restarts == 0
        && (req.http.accept ~ "application/vnd.ezplatform.invalidate-token"
            || req.http.x-backend-purge-token
        )
    ) {
        return (synth(400));
    }

    if (req.restarts == 0 && req.method == "PURGE") {
        set req.http.accept = "application/vnd.ezplatform.invalidate-token";

        set req.url = "/http-cache/purgetoken";

        // Force the lookup
        return (hash);
    }

    // Rebuild the original request which now has the purge token.
    if (req.restarts > 0
        && req.http.accept == "application/vnd.ezplatform.invalidate-token"
    ) {
        set req.url = "/";
...
sub vcl_deliver {
    // On receiving the purge token response, copy the purge token to the original
    // request and restart.
    if (req.restarts == 0
        && resp.http.content-type ~ "application/vnd.ezplatform.invalidate-token"
    ) {
        set req.http.x-backend-purge-token = resp.http.x-invalidate-token;
